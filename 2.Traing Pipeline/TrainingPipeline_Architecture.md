...
> 本文档基于 `2.Traing Pipeline` 目录下的代码，详细介绍了整个医学图像自动分割平台的架构设计、模块职责、数据流转和关键实现细节。
...
- **生产级推理**：预插值加速、显存监控、跨平台适配（Windows/Linux）
...
| `easy_predict_with_preresample` | 生产部署 | 外部预插值+后插值 | **快3-10x** | 较低 |
