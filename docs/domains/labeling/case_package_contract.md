...
| `review_label_map.yaml` | 规定本次标注交换文件中的整数标签 |
...
2. 只有 Mask 体素数组交换稳定时，再增加外部格式脚本和 `.npy` 文件桥接。
...
submissions/{review_id}/buffers/{image_id}/{organ}.npy
...
| 问题 | 验证结果怎样影响实现 |
| --- | --- |
| Mimics 21.0 是否有稳定直接文件路径 | 决定是否需要 `.npy` 文件桥接 |
